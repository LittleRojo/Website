...
function jsonFlickrFeed(json) {
	var tags;
  	$.each(json.items, function(i, item) {
		...
		mesh.position.x -= 100;
               	mesh.position.z = (z - 1400);		
               	window.background.scene.add( mesh );
	
		tags = item.tags;
		z = mesh.position.z;
  	});
	if(window.background.tojo.looper < 10) {
		$.ajax({
...
tojo3.prototype.renderNextFrame = function() {
	for(var i=0; i<window.background.scene.children.length; i++) {		
		...
		if(window.background.scene.children[i].position.z > -1000 && window.background.scene.children[i].position.z % 15 == 0) {
			window.background.scene.children[i].scale.x = window.background.scene.children[i].scale.x + .1;
			window.background.scene.children[i].scale.y = window.background.scene.children[i].scale.y + .1;
			window.background.scene.children[i].position.x = window.background.scene.children[i].position.x - .05;
			window.background.scene.children[i].position.y = window.background.scene.children[i].position.y - .05;
		}
		if(window.background.scene.children[i].position.z > 2500) {
                        window.background.scene.children[i].scale.x = 1;
                        window.background.scene.children[i].scale.y = 1;
                }
