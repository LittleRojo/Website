...
_2.prototype.init = function() {
	...
	for ( var i = 0; i < 50; i ++ ) {
		...
		particle.position.y = Math.random() * 1200 - 800;
		particle.position.z = Math.random() * 1200 - 800;
		particle.scale.x = particle.scale.y =  40 + 30;
...
_2.prototype.render = function() {
	...
	window.ojo2.theta += 1;
	window.ojo2.camera.position.x = window.ojo2.radius * Math.cos( THREE.Math.degToRad( window.ojo2.theta ) );
	window.ojo2.camera.position.y = window.ojo2.radius * Math.cos( THREE.Math.degToRad( window.ojo2.theta ) );
	window.ojo2.camera.position.z = window.ojo2.radius * Math.sin( THREE.Math.degToRad( window.ojo2.theta ) );
